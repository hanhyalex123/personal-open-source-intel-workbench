...
5. 如果证据不足，要明确说证据不足
6. 如果问题询问“最近更新了几次/最新动向/后续方向”，必须明确回答：
   - 观察窗口内大约有几次更新或几类关键变化
   - 最新一次变化是什么
   - 主要方向 2 到 4 条
   - 后续发展方向或风险判断
7. 不要给泛泛的“建议查知乎/媒体报道”，下一步建议必须基于证据和项目本身
8. Markdown 报告优先使用这些章节：
   - `## 结论摘要`
   - `## 更新节奏`
   - `## 主要方向`
   - `## 后续方向`
...
输出 JSON，字段如下：
- report_markdown: 字符串，Markdown 研究报告，至少包含 `## 结论摘要`、`## 主要方向`，并尽量包含 `## 更新节奏`、`## 后续方向`
